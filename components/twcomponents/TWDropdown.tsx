...
const TWDropdown = (props : {title : string, actions: string[], onSelect : any}) => {
  ...
  useEffect(() => {
    const init = async () => {
      const { Dropdown, Input, initTE } = await import("tw-elements");
  ...
  <ul
    className="absolute w-full z-[1000] float-left m-0 hidden min-w-max list-none overflow-hidden rounded-lg border-none bg-white bg-clip-padding text-left text-base shadow-lg dark:bg-neutral-700 [&[data-te-dropdown-show]]:block"
    aria-labelledby="dropdownMenuButton1"

    data-te-dropdown-menu-ref>
      
      {props.actions.map((val: any) => (
                    <li >
                    <a
